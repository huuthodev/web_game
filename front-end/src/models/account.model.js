...
module.exports = function () {
  ...
  // Get username role 0
  this.getUsername = async (result) => {
    const pool = await connect;
    const sqlString = "SELECT * FROM Account where Role = 0";
    return await pool.request().query(sqlString, (err, acc) => {
      if (acc.recordset.length > 0) {
        const accs = acc.recordset;
        result(null, {
          success: true,
          message: "Get account successfully",
          accs,
  ...
  // Get all username
  this.getAllUsername = async (result) => {
    const pool = await connect;
  ...
  // update role account
  this.updateRole = async (user, result) => {
    const pool = await connect;
    const sqlString =
      "UPDATE Account SET Role = @role WHERE ID_DLer=@varId";
    // Hàm
    return await pool
      .request()
      .input("role", sql.Int, user.Role)
