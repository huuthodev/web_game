...
module.exports = function () {
  ...
  this.getNewsProduct = async (result) => {
    ...
    return await pool.request().query(sqlString, (err, product) => {
      if (product.recordset.length > 0) {
        const top4products = product.recordset;
        result(null, {
          success: true,
  ...
  //
  this.getProductByName = async (Name, result) => {
    const pool = await connect;
    const sqlString = "SELECT * FROM Product WHERE [Name] like +'%'+@varName+'%'";
    return await pool
      .request()
 ...
 this.createNewProduct = async (newProduct, result) => {
  const pool = await connect;
